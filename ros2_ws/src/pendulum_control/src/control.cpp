  ...
  double wrap_pendulum(double raw)
{
  ...
  void joint_state_callback(const sensor_msgs::msg::JointState::SharedPtr msg)
  {
    ...
    {
      // ── BALANCE MODE: your PID + motor state feedback ──────────────────
      double error = set_point_ - motor_pos;   // arm position setpoint
      double p_term  = kp_ * error;
      integral_ = std::clamp(
        integral_ + (error + prev_error_) * T / 2.0, -1.0, 1.0);
      double i_term  = ki_ * integral_;
      double d_term  = kd_ * -motor_vel;

      // Pendulum stabilisation terms (pendulum near 0 = upright)
      double pend_p  = -kp_ * alpha;        // push arm to keep pendulum up
      double pend_d  = -kd_ * alpha_dot;
